...
export const full_benchmarks = async (
  ...
  console.log(
    `Running BLS12-377 MSM benchmarks for powers ${START_POWER} to ${END_POWER} (inclusive)`,
  );
  ...
  for (let power = START_POWER; power <= END_POWER; power++) {
    console.log(
      `Running ${NUM_RUNS + 1} invocations of compute_msm() for 2^${power} inputs, please wait...`,
    );
    ...
    if (msm.x !== expected.x || msm.y !== expected.y) {
      console.error(
        `WARNING: the result of compute_msm is incorrect for 2^${power}`,
      );
    }
