...
describe("matrices", () => {
  ...
  describe("sparse matrix functions", () => {
    ...
    it("test_smtvp", async () => {
      ...
      let ell_sparse_matrix = await new ELLSparseMatrix(
        [],
        [],
        [],
      ).dense_to_sparse_matrix(dense_matrix);
      let csr_sparse_matrix = await new CSRSparseMatrix(
        [],
        [],
        [],
      ).ell_to_csr_sparse_matrix(ell_sparse_matrix);
      ...
      let smtvp_result = await csr_sparse_matrix.smtvp_test(vec);
    ...
    it("test_transpose", async () => {
      ...
      let ell_sparse_matrix_transposed = await new ELLSparseMatrix(
        [],
        [],
        [],
      ).dense_to_sparse_matrix(dense_transposed);
      let expected_csr_transposed = await new CSRSparseMatrix(
        [],
        [],
        [],
      ...
      let csr_transposed = await csr_sparse_matrix.transpose_test();
