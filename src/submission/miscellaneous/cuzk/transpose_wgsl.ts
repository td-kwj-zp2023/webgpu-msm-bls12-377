...
import { u8s_to_numbers_32, numbers_to_u8s_for_gpu } from '../../implementation/cuzk/utils'
import {
    get_device,
    create_and_write_sb,
    create_bind_group,
...
export async function transpose(
    ...
    console.log(`transpose_serial_gpu took ${elapsed_gpu}ms on ${num_cols} x ${num_rows} elements`)
    ...
    for (let i = 0; i < csc_val_idxs_result.length; i ++) {
        assert(dm_sparse_vals[csc_val_idxs_result[i]] === transposed_dm_sparse_vals[i])
    }
