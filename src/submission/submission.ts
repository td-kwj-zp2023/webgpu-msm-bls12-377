...
export const compute_msm = async (
  ...
  const bucket_sum_coord_bytelength =
    (num_columns / 2 + 1) * num_words * 4 * 16;
  ...
  const bucket_reduction_shader =
    shaderManager.gen_bucket_reduction_shader(b_workgroup_size);
  ...
  const subtask_sum_data = await read_from_gpu(device, commandEncoder, [
    subtask_sum_x_sb,
    subtask_sum_y_sb,
    subtask_sum_t_sb,
    subtask_sum_z_sb,
  ...
  const x_mont_coords = u8s_to_bigints(
    subtask_sum_data[0],
    num_words,
    word_size,
  );
  const y_mont_coords = u8s_to_bigints(
    subtask_sum_data[1],
    num_words,
    word_size,
  );
  const t_mont_coords = u8s_to_bigints(
    subtask_sum_data[2],
    num_words,
    word_size,
  );
  const z_mont_coords = u8s_to_bigints(
    subtask_sum_data[3],
    num_words,
    word_size,
  );
  ...
  console.log(
    `Final steps (reading subtask sums, conversion out of Montgomery form, and Horner's rule) took ${elapsed}ms`,
  );
...
export const bucket_aggregation = async (
  ...
  if (
    ...
    for (let subtask_idx = 0; subtask_idx < num_subtasks; subtask_idx++) {
      ...
      const points: ExtPointType[] = [];
      for (
        let i = subtask_idx * num_columns;
        i < subtask_idx * num_columns + num_columns;
        i++
      ) {
        points.push(
